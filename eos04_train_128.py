...
eos04_train_128.py
==================
128 x 128 pixel patch version of eos04_train.py.

WHY 128 instead of 256?
  - Each EOS-04 MRS pixel = 18 m resolution
  - 128x128 patch = 2.3 km x 2.3 km  (vs 4.6 km for 256x256)
  - Smaller patches capture finer land-cover boundaries
  - ~4x more patches from the same scene → more training data
  - Better precision on mixed/urban boundary pixels
  - Model still resizes to 224x224 internally — no architecture change

Algorithm is IDENTICAL to eos04_train.py.
Only the input folder (patches_128) and output folder (model/EOS04_128) differ.

Input layout expected
---------------------
  patches_128/03JUN2026/          ← generated by composite_to_patches_128.py
    OPT1_sigma0/
      patches/                    ← 128x128 PNG files
      OPT1_sigma0_labels_clean.csv
    OPT2_gamma0/
      patches/
...
-----
  ...
  python eos04_train_128.py --composite OPT1_sigma0

  # Train on all composites together
  python eos04_train_128.py --composite all

  # Exclude Unclear patches
  python eos04_train_128.py --composite OPT1_sigma0 --drop-unclear

  # Custom epochs
  python eos04_train_128.py --composite OPT1_sigma0 --epochs 50
"""
...
PATCHES_ROOT = os.path.join(BASE_DIR, "patches_128_L2B", "03JUN2026")
...
# Unclear is kept as a class unless --drop-unclear is passed
CLASS_NAMES_FULL    = ["Dense_forest", "Mix", "Urban", "Unclear"]
CLASS_NAMES_NO_UNC  = ["Dense_forest", "Mix", "Urban"]
...
def load_labels(composite: str, patches_root: str,
                drop_unclear: bool = False) -> pd.DataFrame:
    """
    ...
    for name in targets:
        ...
        # Try multiple locations and filename patterns in priority order.
        # This handles files placed in the root folder, inside the composite
        # subfolder, with different names, or as .xlsx instead of .csv.
        short = name.replace("OPT1_sigma0","OPT1") \
                    .replace("OPT2_gamma0","OPT2") \
                    .replace("OPT3_rvi",   "OPT3")

        candidates = [
            # expected location (standard)
            os.path.join(patches_root, name, f"{name}_labels_clean.csv"),
            os.path.join(patches_root, name, f"{name}_labels.csv"),
            # root folder, various naming patterns
            os.path.join(patches_root, f"{name}_labels_clean.csv"),
            os.path.join(patches_root, f"{name}_labels.csv"),
            os.path.join(patches_root, f"patches_128autolabels_{short}.csv"),
            os.path.join(patches_root, f"patches_128autolabels_{short}.xlsx"),
            os.path.join(patches_root, f"patches_autolabels_{short}.csv"),
            os.path.join(patches_root, f"patches_autolabels_{short}.xlsx"),
            # any xlsx in root that contains the short composite name
        ]

        # also glob for any file with composite keyword in root / subfolder
        import glob as _glob
        for folder in [patches_root, os.path.join(patches_root, name)]:
            for ext in ["csv", "xlsx"]:
                candidates += _glob.glob(
                    os.path.join(folder, f"*{short}*.{ext}"))
                candidates += _glob.glob(
                    os.path.join(folder, f"*{name}*.{ext}"))

        # deduplicate while preserving order
        seen = set(); unique_cands = []
        for c in candidates:
            if c not in seen:
                seen.add(c); unique_cands.append(c)
        ...
        if label_path is None:
            print(f"  [WARN] No label file found for '{name}'. Tried:")
            for c in unique_cands[:8]:
                print(f"         {c}")
            continue
        ...
        if ext == ".xlsx":
            ...
            print(f"  Loaded (xlsx) {label_path}  →  {len(df)} rows")
        else:
            df = pd.read_csv(label_path)
            print(f"  Loaded (csv)  {label_path}  →  {len(df)} rows")
        ...
        if "label" not in df.columns:
            ...
            alt_label_cols = ["Label", "class", "Class", "CLASS",
                               "category", "Category", "land_cover"]
            found = [c for c in alt_label_cols if c in df.columns]
            if found:
                df = df.rename(columns={found[0]: "label"})
                print(f"    Renamed column '{found[0]}' → 'label'")
            else:
        ...
        # build absolute PNG path — handle full paths, relative paths,
        # or just bare filenames stored in the xlsx/csv
        composite_dir = os.path.join(patches_root, name)

        def resolve_png_path(p):
            p = str(p).strip()
            if os.path.isabs(p) and os.path.isfile(p):
                return p                                    # already absolute
            full = os.path.join(composite_dir, p)
            if os.path.isfile(full):
                return full                                 # relative to composite dir
            # try treating p as bare filename inside patches/ subfolder
            bare = os.path.join(composite_dir, "patches", os.path.basename(p))
            if os.path.isfile(bare):
                return bare
            return full                                     # best guess

        if "png_path" in df.columns:
            df["abs_png_path"] = df["png_path"].apply(resolve_png_path)
        elif "patch_id" in df.columns:
            # reconstruct path from patch_id if png_path column is missing
            df["abs_png_path"] = df["patch_id"].apply(
                lambda pid: os.path.join(composite_dir, "patches",
                                         f"{pid}.png"))
        else:
            raise ValueError(
                f"Label file {label_path} has neither 'png_path' nor "
                f"'patch_id' column. Columns found: {list(df.columns)}"
            )
    ...
    # Drop Unclear if requested
    if drop_unclear:
        before = len(merged)
        merged = merged[merged["label"] != "Unclear"].copy()
        print(f"  Dropped {before - len(merged)} Unclear patches")

    # Decide class list
    class_names = CLASS_NAMES_NO_UNC if drop_unclear else CLASS_NAMES_FULL
...
#   Fusion head  : concat → Linear → 3 or 4 classes
#
# WHY multi-input?
...
def plot_cm(labels, preds, class_names, model_dir):
    # FIX: explicitly pass all label ids so the matrix always has a row/col
    # per class_names entry, even if a class (e.g. Unclear) never appears
...
def main():
    ...
    parser.add_argument("--drop-unclear", action="store_true",
                        help="Exclude Unclear patches from training")
    args = parser.parse_args()

    print("=" * 60)
    print("EOS-04 SAR PATCH CLASSIFIER — TRAINING (128x128 patches)")
    print(f"Composite   : {args.composite}")
    print(f"Patch size  : 128 x 128 pixels  (2.3 km x 2.3 km @ 18m resolution)")
    print(f"Drop Unclear: {args.drop_unclear}")
    ...
    # ── Load labels ───────────────────────────────────────────────────────
    print("\n[1] Loading labels ...")
    df, class_names = load_labels(
        args.composite, PATCHES_ROOT, args.drop_unclear
    )
    ...
    # FIX: value_counts() only returns counts for label_ids that actually
    # appear in the train split. If a class (e.g. "Unclear") has zero
    # patches, it's silently missing from the array, so the weight tensor
    # ends up shorter than num_classes and cross_entropy throws a shape
    # mismatch. Build counts explicitly for every class id 0..num_classes-1,
    # giving absent classes a neutral placeholder count.
    counts_series = train_df["label_id"].value_counts()
    counts = np.zeros(num_classes, dtype=np.float64)
    for lid, cnt in counts_series.items():
        counts[int(lid)] = cnt
    counts = np.where(counts == 0, 1.0, counts)  # absent class → neutral weight

    weights = torch.tensor(1.0 / counts, dtype=torch.float32).to(DEVICE)
    weights = weights / weights.sum() * num_classes

    weight_dict = {class_names[i]: round(float(weights[i]), 3)
    ...
    # FIX: same absent-class issue as the loss weights — final_labels/preds
    # may contain fewer distinct values than len(class_names) if a class
    # (e.g. Unclear) has zero validation samples. Passing labels= explicitly
    # forces sklearn to report all classes instead of inferring the count
    # from what's present, which caused the mismatch with target_names.
    ...
    print("\n" + "=" * 60)
    print("TRAINING COMPLETE  (128x128 patch model)")
    ...
    print(f"\n  To predict on full scene with this model run:")
    print(f"  python eos04_predict_scene_128.py --model-dir {MODEL_DIR}")
    print("=" * 60)
