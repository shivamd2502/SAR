#!/usr/bin/env python3
"""
composite_to_patches_128.py
============================
128 x 128 pixel version of composite_to_patches.py.

WHY 128 instead of 256?
  Each EOS-04 MRS pixel = 18 m ground resolution.
  256 x 256 patch covers 4.6 km x 4.6 km  — may mix multiple land covers.
  128 x 128 patch covers 2.3 km x 2.3 km  — finer spatial detail per patch,
  better class separability, ~4x more patches from the same scene.

Single script that:
  1. Reads every composite GeoTIFF from composites/03JUN2026
     (OPT1 sigma0, OPT2 gamma0, OPT3 RVI, OLD baseline + standalone
      RVI / DPDI layers)
  2. Cuts each composite into 128x128 PNG patches using a sliding grid
  3. Writes a Shapefile (.shp) for EVERY composite — each feature in
     ...
     in QGIS whether that patch is useful for labelling a particular class
  4. Writes one master SHP that covers all composites
...
Usage examples
--------------
  # default 128x128 patches, 128 stride (no overlap)
  python composite_to_patches_128.py ^
      --composite-dir "composites\\03JUN2026" ^
      --output-dir    "patches_128\\03JUN2026" ^
      --patch-size 128 --stride 128

  # 128x128 with 50% overlap (more training patches, better generalisation)
  python composite_to_patches_128.py ^
      --composite-dir "composites\\03JUN2026" ^
      --output-dir    "patches_128\\03JUN2026" ^
      --patch-size 128 --stride 64

  # skip patches with less than 70% valid pixels
  python composite_to_patches_128.py ^
      --composite-dir "composites\\03JUN2026" ^
      --output-dir    "patches_128\\03JUN2026" ^
      --patch-size 128 --stride 128 --min-valid 70
"""
...
def main(argv=None):
    ...
    p.add_argument("--patch-size", type=int, default=128,
                    help="Patch height and width in pixels (default 128)")
    p.add_argument("--stride",     type=int, default=None,
                    help="Stride in pixels. Defaults to patch-size (no overlap). "
                         "Use half patch-size (64) for 50%% overlap.")
...
Output folder (128x128 patches): {args.output_dir}

Files per composite:
  {args.output_dir}/<composite_name>/
      patches/           ← PNG files (128x128, one per patch)
      <composite>.shp    ← shapefile (open this in QGIS)

Master shapefile (all composites):
  {args.output_dir}/master_all_composites.shp

NOTE: 128x128 patches produce ~4x more patches than 256x256.
Each patch covers 2.3 km x 2.3 km (vs 4.6 km for 256x256).
Smaller patches = finer class boundaries = better precision.
